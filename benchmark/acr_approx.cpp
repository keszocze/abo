...
static void acr_adders(benchmark::State& state) {
    ...
    std::string adder_configuration = approximate_adder_name(adder, state.range(1), state.range(2), state.range(3));
    ...
    for (auto _ : state) {
        ...
        std::vector<BDD> approximate_adder = get_approximate_adder(mgr, adder, state.range(1), state.range(2), state.range(3));
        state.ResumeTiming();

        abo::benchmark::compute_error_metric(mgr, correct, approximate_adder, ErrorMetric::AVERAGE_RELATIVE_APPROX);
    }
}
