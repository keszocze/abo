...
static void benchmark_iscas85(benchmark::State& state)
{
    ...
    state.SetLabel(file + "(" + rounding_text + "->" + std::to_string(state.range(3) + 1) + ") - " +
                   error_metric_name);
    for (auto _ : state)
    {
        state.PauseTiming();
        ...
        for (const BDD& orig : original)
        {
            ...
            case ROUND_UP:
                rounded.push_back(
                    abo::operators::superset_heavy_child(mgr, orig, state.range(3), 10000));
                break;
            case ROUND_DOWN:
                rounded.push_back(
...
BENCHMARK(benchmark_iscas85)->Unit(benchmark::kMillisecond)->Apply([](auto* b) {
    std::vector<ErrorMetric> metrics = {ErrorMetric::ERROR_RATE, ErrorMetric::WORST_CASE,
                                        ErrorMetric::AVERAGE_CASE};
