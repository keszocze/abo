...
static void benchmark_ace(benchmark::State& state)
{
    ...
    state.SetLabel(file + (state.range(2) ? "(round_up->" : "(round_down->") +
                   std::to_string(state.range(1) + 1) + ")");
    ...
    for (const BDD& orig : original)
    {
        ...
        {
            rounded.push_back(
                abo::operators::superset_light_child(mgr, orig, state.range(1), 10000));
        }
        else
        {
    ...
    while (state.KeepRunningBatch(BATCH_SIZE))
    {
        ...
        {
            abo::benchmark::compute_error_metric(mgr, original, rounded,
                                                 abo::benchmark::ErrorMetric::AVERAGE_CASE);
