...
static void benchmark_operators(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        for (const BDD& orig : original)
        {
            ...
            {
            case SUPERSET_HEAVY:
                rounded.push_back(abo::operators::superset_heavy_child(mgr, orig, state.range(2),
                                                                       state.range(2)));
                break;
            case SUBSET_LIGHT:
                rounded.push_back(
                    abo::operators::subset_light_child(mgr, orig, state.range(2), state.range(2)));
                break;
            case SUPERSET_LIGHT:
                rounded.push_back(abo::operators::superset_light_child(mgr, orig, state.range(2),
                                                                       state.range(2)));
                break;
            case SUBSET_HEAVY:
                rounded.push_back(
                    abo::operators::subset_heavy_child(mgr, orig, state.range(2), state.range(2)));
                break;
            case ROUND_BEST:
                rounded.push_back(
...
BENCHMARK(benchmark_operators)->Unit(benchmark::kMillisecond)->Apply([](auto* b) {
    for (auto op : {SUPERSET_HEAVY, SUBSET_LIGHT, SUPERSET_LIGHT, SUBSET_HEAVY, ROUND_FULL,
                    ROUND_BEST, COFACTOR_POSITIVE, COFACTOR_NEGATIVE})
    {
        for (int i = 1; i < 41; i++)
        {
