...
int main() {
    ...
    for (auto [bits, max_ace] : std::vector<std::pair<int, int>>{{8, 7}, {16, 35}, {32, 9000}}) {
        ...
        abo::nsga2minimization::MinimizationResult result = abo::nsga2minimization::run_genetic_minimization(mgr,
                                                                                                             function,
                                                                                                             MAX_ERROR_RATE,
                                                                                                             max_ace, true);
        std::cout <<bits<<", "<<mgr.nodeCount(function)<<", "<<result.node_count<<", "
            <<result.error_rate<<", "<<result.average_case_error<<", "<<abo::minimization::format_time(result.computation_time)<<std::endl;
    }
