...
BDDMinimizationProblem::BDDMinimizationProblem(const Cudd &mgr, const std::vector<BDD> &original, float max_error_rate, bool er_and_ace, float max_ace)
    : mgr(&mgr),
      original(&original),
...
std::vector<std::pair<std::size_t, std::size_t>> BDDMinimizationProblem::compute_support_positions(const std::vector<BDD> &function) {
...
std::pair<pagmo::vector_double, pagmo::vector_double> BDDMinimizationProblem::get_bounds() const {
    return {pagmo::vector_double(support_positions.size(), 0), pagmo::vector_double(support_positions.size(), 2)};
}

pagmo::population BDDMinimizationProblem::make_population(std::size_t population_size, std::size_t initialization_count) const {
...
std::vector<BDD> BDDMinimizationProblem::function_from_individuum(const pagmo::vector_double &parameters) const {
...
pagmo::vector_double BDDMinimizationProblem::fitness(const pagmo::vector_double &parameters) const {
    ...
    // if the created function is greater than the original, we do not need to compute the error metrics as this indivuum is guaranteed
    // to not lie in any reasonable pareto front as it is dominated by the original un-approximated function
    ...
    if (er_and_ace) {
        average_case_error = double(abo::error_metrics::average_case_error(*mgr, function, individuum_function));
    }

    // punish error values above the maximum error metrics (the exact multipliers do not matter here)
