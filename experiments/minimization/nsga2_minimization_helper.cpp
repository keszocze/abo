                                            ...
                                            bool use_both_metrics)
{
    ...
    // the number of generations to run in one step, if no further improvement was found, the
    ...
    BDDMinimizationProblem problem(mgr, function, max_error_rate, use_both_metrics,
                                   max_average_case_error);

    //  convert the mutation count to a mutation probability
    float mp = MUTATION_COUNT / problem.get_nix();
    pagmo::algorithm algorithm(
        pagmo::nsga2(NUM_GENERATIONS, CROSSOVER_PROBABILITY, CROSSOVER_ETA, mp, MUTATION_ETA));
    ...
    while (true)
    {
        ...
        for (auto ind : pop.get_x())
        {
            ...
            // if a function with a smaller node count within the error metric constraints is found,
            // save it
            if (fit[1] < max_error_rate && (!use_both_metrics || fit[2] < max_average_case_error) &&
                fit[0] < best_result.node_count)
            {
