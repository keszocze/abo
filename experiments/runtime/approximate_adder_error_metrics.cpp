...
static void generate_adders(benchmark::State& state)
{
    ...
    std::string adder_configuration =
        approximate_adder_name(adder, state.range(2), state.range(3), state.range(4));
    ...
    for (auto _ : state)
    {
        ...
        std::vector<BDD> approximate_adder =
            get_approximate_adder(mgr, adder, state.range(2), state.range(3), state.range(4));
        state.ResumeTiming();
...
BENCHMARK(generate_adders)->Unit(benchmark::kMillisecond)->Apply([](auto* b) {
    std::vector<ErrorMetric> metrics = {
        ErrorMetric::WORST_CASE,       ErrorMetric::WORST_CASE_RELATIVE_APPROX,
        ErrorMetric::AVERAGE_CASE,     ErrorMetric::AVERAGE_RELATIVE_APPROX,
        ErrorMetric::MEAN_SQUARED,     ErrorMetric::ERROR_RATE,
