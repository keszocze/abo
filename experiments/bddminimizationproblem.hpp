 ...
 * It creates and handles individuals that represent approximated functions for the use in a genetic algorithm
 * This class encodes the kind of approximations as follows:
 * For each BDD in the function and each variable that is present in the support of that function,
 * the genetic algorithm is given the choice to use an approximation operator operating on exactly that BDD and variable level.
 * For each of these possibilities, one integer value is stored in the individuum,
 * with 0 representing no approximation (the default state) and higher values representing the different approximations
 */
...
public:
    ...
    BDDMinimizationProblem(const Cudd &mgr, const std::vector<BDD> &original, float max_error_rate, bool er_and_ace, float max_ace);
     ...
     * @param initialization_count The number of approximations used in each individuum. They are randomly distributed
     * @return The created population
     */
    pagmo::population make_population(std::size_t population_size, std::size_t initialization_count) const;
...
private:
    ...
    static std::vector<std::pair<std::size_t, std::size_t>> compute_support_positions(const std::vector<BDD> &function);
