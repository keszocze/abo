...
boost::multiprecision::cpp_dec_float_100 mean_squared_value(const std::vector<BDD>& f)
{
    ...
    for (unsigned int i = 0; i < f.size(); i++)
    {
        for (unsigned int b = i; b < f.size(); b++)
        {
            ...
            sum += (one << i) * (one << b) * boost::multiprecision::uint256_t(minterms) *
                   doubling_factor;
        }
    }
...
boost::multiprecision::cpp_dec_float_100 average_case_error_add(const Cudd& mgr,
...
boost::multiprecision::cpp_dec_float_100 mean_squared_error_add(const Cudd& mgr,
                                                                const std::vector<BDD>& f,
                                                                const std::vector<BDD>& f_hat,
                                                                const NumberRepresentation num_rep)
{
