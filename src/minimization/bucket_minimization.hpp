...
namespace abo::minimization {
     ...
     * @param level_end The last level the operator should be applied at. b is approximated for all variable levels between level_start and level_end
     ...
     * one approximation function is created which applies the operator at exactly that level to the given BDD.
     * @param function The original function that is later approximated. Only the support size and the number of bits are used,
     ...
     * one approximation function is created which applies the operator at exactly that level to all bits in the input.
    ...
    /**
     * @brief generate_random_operators Generates a set of approximation operator functions of size count.
     * This is very similar to generate_single_bdd_operators, except that the variable level, bit and operator are randomly
     * selected upon calling the lambda instead of being hardcoded. The same function is therefore repeated in the result.
     * count (and therefore the size of the result) is only used to influence the bucket based minimization algorithm.
     * @param function The original function that is later approximated. Only the support size and the number of bits are used,
     * the function to approximate is later passed to each OperatorFunction individually
     * @param operators The set approximation operators to use
    ...
    //! A struct holding the information about a bucket. This is also returned as the result of the minimization procedure
    struct Bucket {
        //! The function currently occupying this bucket. It represents the currently best result found for this bucket
        std::vector<BDD> function;
        //! The number of nodes of the function
        std::size_t bdd_size;
        //! The value of every metric computed for the function
        std::vector<double> metric_values;
        //! For each operator given to the optimization procedure, this stores wether it can still be applied to the function
     ...
     * A set of buckets is created, quantizing the space of the given error metrics up the maximum error bound.
     * Each bucket can only be inhabited by one BDD, the one with the lowest node cound found in this error metric range.
     * When a better result for a bucket is found, the given approximation operator functions are applied to it to create more (possibly better) approximations.
     * @param mgr The cudd node manager to create nodes in
     * @param function The function to approximate
     * @param metrics The metrics used for the minimization procedure. It is recommended to keep the number of metrics fairly low to avoid an
     * explosion in the number of buckets created by the procedure
     * @param operators The approximation operators to be used. Can for example be created by generate_single_bdd_operators and similar helper functions
     * @param populate_all_buckets When a better approximation for a bucket is found, this flag determines if the result is written into the exact bucket only (false)
     * or all buckets with higher error metrics and larger node counts.
     * @return A list of buckets created by the procedure. They represent a pareto front of the minimization task
     */
