...
namespace abo::minimization {
    ...
    std::size_t reduce_multi_dim_index(const std::vector<std::size_t> &index, const std::vector<std::size_t> &bounds) {
        std::size_t result = 0;
    ...
    std::vector<std::size_t> create_multi_dim_index(std::size_t index, const std::vector<std::size_t> &bounds) {
    ...
    std::vector<Bucket> bucket_greedy_minimize(Cudd &mgr,
        ...
        while (test.size() > 0) {
            ...
            auto smallest_pos = *std::min_element(test.begin(), test.end(), [](const std::vector<std::size_t> &a, const std::vector<std::size_t> &b) {
    ...
    BDD apply_operator(const Cudd &mgr, BDD &b, Operator op, unsigned int level_start, unsigned int level_end) {
        ...
        case Operator::ROUND:
            return abo::operators::round(mgr, b, level_start);
        }
        return b;
    }

    void apply_operator(const Cudd &mgr, std::vector<BDD> &function, Operator op, unsigned int level_start, unsigned int level_end) {
    ...
    std::function<double(Cudd&, const std::vector<BDD>&, const std::vector<BDD>&)> metric_function(ErrorMetric metric) {

        switch (metric) {
        case ErrorMetric::WORST_CASE:
           return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::worst_case_error(mgr, orig, approx)); };
        case ErrorMetric::WORST_CASE_PERCENT:
        return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::worst_case_error_percent(mgr, orig, approx)); };
        case ErrorMetric::WORST_CASE_RELATIVE:
            return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::worst_case_relative_error_add(mgr, orig, approx)); };
        case ErrorMetric::AVERAGE_CASE:
            return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::average_case_error(mgr, orig, approx)); };
        case ErrorMetric::AVERAGE_CASE_RELATIVE:
            return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::average_relative_error_add(mgr, orig, approx)); };
        case ErrorMetric::AVERAGE_CASE_RELATIVE_ADD:
            return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::average_relative_error_add(mgr, orig, approx)); };
        case ErrorMetric::MEAN_SQUARED:
            return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::mean_squared_error(mgr, orig, approx)); };
        case ErrorMetric::ERROR_RATE:
            return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::error_rate(mgr, orig, approx)); };
        case ErrorMetric::AVERAGE_BIT_FLIP:
            return [](Cudd &, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::average_bit_flip_error(orig, approx)); };
        case ErrorMetric::WORST_CASE_BIT_FLIP:
            return [](Cudd &mgr, const std::vector<BDD> &orig, const std::vector<BDD> &approx) { return static_cast<double>(abo::error_metrics::worst_case_bit_flip_error(mgr, orig, approx)); };
        }
